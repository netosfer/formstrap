...
class FormBuilder
{
    ...
    private function renderInput(): string
    {
        ...
        $output = '<input ' . $attrs . '>';
        return $this->wrapperInput($output);
    }
    ...
    {
        extract($this->get('label', 'formInline', 'render', 'required'));

        $class = in_array($render, ['checkbox', 'radio']) ? 'form-check-label' : ($formInline ? 'col-form-label' : 'form-label');
        if ($formInline) {
            $class = join(' ', [$class, 'col-sm-2']);
        }
        if ($required) {
            $class = join(' ', [$class, 'required']);
        }
    ...
    private function wrapperInput(string $input): string
    {
        ...
        $helpText       = $help ? '<div id="help-' . $id . '" class="form-text">' . $this->getText($help) . '</div>' : '';
        $error          = $this->getInputErrorMarkup($name);
        $attrs          = $wrapperAttrs ?? [];
        $attrs['class'] = $this->createAttrsList(
            $attrs['class'] ?? null,
            $formInline ? 'mb-3 row' : 'mb-3'
        );
        $attributes = $this->buildHtmlAttrs($attrs, false);
        $inHtml = $label . $input . $helpText . $error;
        if($formInline){
            $inHtml = $label . '<div class="col-sm-10">' . $input . $helpText . '</div>' . $error;
        }
        return '<div ' . $attributes . '>' . $inHtml . '</div>';
    }

    private function wrapperRadioCheckbox(string $input): string
    {
        extract($this->get('inline', 'name', 'wrapperAttrs', 'values', 'formInline'));
        $attrs = $wrapperAttrs ?? [];
        $attrs['class'] = $this->createAttrsList(
            'row',
        ...
        $label = $this->renderLabel();
        $error = $this->getInputErrorMarkup($name);
        $inHtml = $label .  $input . $error;
        if($formInline){
            $inHtml = $label . '<div class="col-sm-10">' . $input .  '</div>' . $error;
        }
        return '<div ' . $attributes . '>' . $inHtml . '</div>';
    }
    ...
    {
        extract($this->get('name', 'values', 'formData'));
        if ($this->isRadioOrCheckbox()) {
            return $values;
        }
